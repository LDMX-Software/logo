...
/// electron-color: type color, default to bkgd-color, color of electron "beam" in logo
/// dm-color: type color, default to text-color, color of produced DM in logo
...
#let logo(
  ...
  diagram(
    ...
    node(
      ...
      width: if prefix == none { 0.25cm } else { auto },
      name: <prefix>
    ),
    node(
      (rel: (-0.45em, 0em), to: <L>),
    ...
    node(
      //(rel: (-1.2cm, 0cm), to: <target>),
      pos: <prefix.west>,
      name: <beam-entry>
    ),
    node(
      (rel: (0.8cm, +0.5cm), to: <target>),
      name: <beam-exit>
    ),
    node(
      (2,0),
      shape: rect,
      text(size: 32pt, weight: "black")[DM],
      name: <DM>,
    ),
    node(
      (3,0),
